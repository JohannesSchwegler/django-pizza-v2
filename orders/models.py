...
class PizzaName(models.Model):
    name = models.CharField(max_length=32)
    image = models.ImageField(default="no_photo_available.png")
...
class Pizza(models.Model):
    name = models.ForeignKey(
        PizzaName, on_delete=models.CASCADE, related_name="pizzas")
    size = models.ForeignKey(
        Size, on_delete=models.CASCADE, related_name="pizzas")
...
class Sub(models.Model):
    name = models.ForeignKey(
        SubName, on_delete=models.CASCADE, related_name="subs")
    image = models.ImageField(default="no_photo_available.png")
    size = models.ForeignKey(
        Size, on_delete=models.CASCADE, related_name="subs")
    price = models.DecimalField(
        max_digits=4, decimal_places=2, default=Decimal("0.0"))
...
class DinnerPlatter(models.Model):
    name = models.ForeignKey(
        DinnerPlatterName, on_delete=models.CASCADE, related_name="dinner_platters")
    size = models.ForeignKey(
        Size, on_delete=models.CASCADE, related_name="dinner_platters")
    price = models.DecimalField(
        max_digits=4, decimal_places=2, default=Decimal('0.0'))
...
class Order(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="orders", default=0)
    ...
    dinner_platters = models.ManyToManyField(
        DinnerPlatter, blank=True, through="OrderDinnerPlatter")
    ...
    def get_order_price(self):
        ...
        # Pizzas
        price += sum([x.get_price() *
                      x.quantity for x in OrderPizza.objects.filter(order__id=self.id)])

        # Subs
        price += sum([x.get_price() *
                      x.quantity for x in OrderSub.objects.filter(order__id=self.id)])

        # Pastas
        price += sum([x.get_price() *
                      x.quantity for x in OrderPasta.objects.filter(order__id=self.id)])

        # Salads
        price += sum([x.get_price() *
                      x.quantity for x in OrderSalad.objects.filter(order__id=self.id)])

        # Dinner Platters
        price += sum([x.get_price() *
                      x.quantity for x in OrderDinnerPlatter.objects.filter(order__id=self.id)])
